...
求解过程 ：
 1. 将图反向，用 dijstra + heap 求出 t 到所有点的最短距离
，目的是求所有点到点 t 的最短路，用 dis[i] 表示 i 到 t 的最
短路，其实这就是 A* 的启发函数，显然 ：h(n) <= n 到 t 的实
际代价
 2. 定义估价函数。我们定义 g(n) 为从 s 到 n 所花费的代价， h(
n) 为 dis[n] ，显然这符合 A* 算法的要求
 3. 初始化状态。状态中存放当前到达的点 i , fi , gi 。显然，
 fi = gi + dis[i] 。初始状态为 (0, S, dis[S],) ，存入优先
级队列中。
 4. 状态转移。假设当前状态所在的点 u 相邻的点 v ，我们可以
得到转换： (gu, u, fu) --> (gu + w[u][v], v, fu + w[u][v]) 。
 5. 终止条件。每个节点最多入队列 K 次，当 t 出队列 K 次时，即
找到解。
************************************************
@ 接口: void Dij (int s)
@ 功能: 求解从 s 开始的最短路
